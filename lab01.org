...
* DONE 8 duds and 1 armed bomb.
  CLOSED: [2018-05-04 Fri 09:26]
  :LOGBOOK:
  CLOCK: [2018-05-04 Fri 09:22]--[2018-05-04 Fri 09:26] =>  0:04
  :END:
Some idiot forgot to disarm a bomb that is laid in the 'decomisioned' bomb pile.
Unfortunately the only way to tell which bomb is armed is to use a special weighing system.
To add insult to injury we don't know if the armed bomb weighs less or more on the system just that it weighs differently.
...
In class we worked out that you can divide the bombs into three piles with three bombs in each. Placing two of the piles on the
scale has two options, either one side is heavier than the other or they are equal. Given the change made in class where the armed
bomb is known to be heavier rather than just different, either the pile that is shown to ber heavier or the pile not weighed
is the pile that includes the bomb. Now, similar to the first step, two of the bombs are weighed while one is left out. If one
of the bombs on the scale is heavier, it is the armed bomb, otherwise, the bomb left out is the armed one.
